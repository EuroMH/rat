...
def get_data(path: str, profile: str, key, type_of_data):
    ...
    result = ""
    try:
        shutil.copy(db_file, os.path.join(tempfile.gettempdir(), 'temp_db'))
    except Exception as e:
        return result

    conn = sqlite3.connect(os.path.join(tempfile.gettempdir(), 'temp_db'))
    cursor = conn.cursor()
    ...
    conn.close()
    os.remove(os.path.join(tempfile.gettempdir(), 'temp_db'))
    return result
